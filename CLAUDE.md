...
- **데이터 저장소**: Vercel Postgres (`@vercel/postgres`)
  - SQL 기반 CRUD, 서버리스 환경 호환
  - `POST /api/db/init`으로 스키마 초기화
